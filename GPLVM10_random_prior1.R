...
dL.dZ <- function(X, Z, l, alpha, sigma, sigma1, K, dL.dK, Z.normal.prior, groups,muu,sigmaa,cluster) {
  out <- Z
  for (i in 1:nrow(Z)) {
    for (j in 1:ncol(Z)) {
  ...
  Z1<-Z
  q<-dim(Z)[2]
  for(k in 1:(dim(muu)[1]))
  {
  	ei<-eigen(sigmaa[(((k-1)*q+1):(k*q)),])
  	siginv<-ei$vectors[,(ei$values>0)]%*%diag(1/(ei$values[ei$values>0]))%*%t(ei$vectors[,(ei$values>0)])
  	newfn<-function(x,vec=muu[k,],s1=siginv)
  	{
  		return(siginv%*%(x-vec))
  	}
  	Z1[(cluster==k),]<-apply(Z[(cluster==k),],1,newfn)
  }
...
gplvm.L.from.K <- function(Z, X, K, Z.normal.prior=TRUE,muu,sigmaa,cluster) {
  X <- as.matrix(X)
  D <- ncol(X)
  N <- nrow(X)
  K.chol <- chol(K)
  ...
  Z.prior.term <- 0
  dists<-rep(NA,(dim(Z)[1]))
  q<-dim(Z)[2]
  for(k in 1:(dim(muu)[1]))
  {
  	below_ave<- function(x,me=muu[k],s1=diag(sigmaa[(((k-1)*q+1):(k*q)),])) {  
    return(sum((x-me)^2/s1))
    }
    dists[cluster==k]<-apply(Z[(cluster==k),],1,below_ave)
  }
  varsum<-0
  for(k in 1:(dim(muu)[1]))
  {
  	varsum<-varsum+log(prod(diag(sigmaa[(((k-1)*q+1):(k*q)),])))*sum(cluster==k)
  }
  if (Z.normal.prior) {
    Z.prior.term <- - length(Z)/2 * log(2 * pi) - 1 / (2) * sum(dists)
  }
  Z.prior.term<-Z.prior.term-1/2*varsum
  return(-D * N * log(2 * pi) / 2 - D * K.log.det / 2 - 1/2 * sum(K.inv.X * X) + Z.prior.term)
}



gplvm.L <- function(Z, X, l, alpha, sigma,sigma1, Z.normal.prior=TRUE,groups,muu,sigmaa,cluster) {
  K <- gplvm.SE(Z, l, alpha, sigma,sigma1,groups)
  return(gplvm.L.from.K(Z, X, K, Z.normal.prior,muu,sigmaa,cluster))
}



gplvm.f <- function(par, X,groups, Z.normal.prior,muu,sigmaa,cluster) {
  l <- par[1]
  alpha <- par[2]
  sigma <- par[3]
  sigma1 <- par[4]
  Z <- matrix(par[-(1:4)], nrow=nrow(X))
  gplvm.L(Z, X, l, alpha, sigma, sigma1, Z.normal.prior,groups,muu,sigmaa,cluster)
}



gplvm.gr <- function(par, X,groups, Z.normal.prior,muu,sigmaa,cluster) {
  l <- par[1]
  alpha <- par[2]
  sigma <- par[3]
  sigma1<-par[4]
  ...
  c(dL.dl(X, Z, l, alpha, sigma, sigma1, K, dL.dK,groups),
    ...
    as.numeric(dL.dZ(X, Z, l, alpha, sigma, sigma1, K, dL.dK, Z.normal.prior,groups,muu,sigmaa,cluster))
  )
}


gplvm.hp.f <- function(par, Z, X,groups, Z.normal.prior,muu,sigmaa,cluster) {
  l <- par[1]
  alpha <- par[2]
  sigma <- par[3]
  sigma1<-par[4]
  gplvm.L(Z, X, l, alpha, sigma,sigma1, Z.normal.prior=Z.normal.prior,groups,muu,sigmaa,cluster)
}



gplvm.hp.gr <- function(par, Z, X, groups, Z.normal.prior,muu,sigmaa,cluster) {
  l <- par[1]
  alpha <- par[2]
  sigma <- par[3]
  sigma1<-par[4]
...
#	}
  ...
  par.init <- c(1, 1, 1,1)
  par.init.L <- gplvm.hp.f(par.init, Z=Z, X=X,groups, Z.normal.prior=Z.normal.prior,muu,sigmaa,cluster)
  #first optimize the hyperparams for the initial Z
  for (i in seq(num.init.params)) {
    test.par.init <- rgamma(4, 1)
    test.par.init.L <- gplvm.hp.f(test.par.init, Z=Z, X=X,groups, Z.normal.prior=Z.normal.prior,muu,sigmaa,cluster)
