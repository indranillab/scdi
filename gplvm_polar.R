...
function(X, initial_config = NULL, k=2, initial_dims=30, perplexity=30, max_iter = 1000, min_cost=0, epoch_callback=NULL,whiten=TRUE, epoch=100 ){
	...
	for (iter in 1:max_iter){
		...
		#}
		K<-alpha*ydata%*%t(ydata)+(1/beta)*diag((dim(ydata)[1]))+gamma*matt1
		K1<-solve(K)
		grads<--(K1%*%matt%*%K1%*%ydata-(dim(X)[2])*K1%*%ydata)
		gradsR<-rowSums(grads*cbind(cos(Theta),sin(Theta)))
		gradsTheta<-rowSums(grads*cbind(-R*sin(Theta),R*cos(Theta)))
		grads1<-(dim(X)[2])*sum(diag(K1%*%(ydata%*%t(ydata))))/2-sum(diag(K1%*%ydata%*%t(ydata)%*%K1%*%matt))/2
		...
		gainsR = ((gainsR * .2) * abs(sign(gradsR) != sign(incsR)) +
				 gainsR * .8 * abs(sign(gradsR) == sign(incsR)))
		gainsTheta = ((gainsTheta * .2) * abs(sign(gradsTheta) != sign(incsTheta)) +
				 gainsTheta * .8 * abs(sign(gradsTheta) == sign(incsTheta)))
		...
		incsR = momentum * incsR - epsilonR * (gainsR * gradsR)
		incsTheta = momentum * incsTheta - epsilonTheta * (gainsTheta * gradsTheta)
		...
		#for(j in 1:(dim(ydata)[2]))
		#{
		#	ydata[,j]<-ydata[,j]/sd(ydata[,j])
		#}
		#V<-var(ydata)
		#ei<-eigen(V)
		#ydata<-ydata%*%(ei$vectors)%*%diag(1/sqrt(ei$values))%*%t(ei$vectors)
      
		#Theta<-Theta-mean(Theta)
		#for(i in 1:(dim(ydata)[1]))
		#{
		#	ydata[i,1]<-R[i]*cos(Theta[i])
		#	ydata[i,2]<-R[i]*sin(Theta[i])
		#}
		alpha<-alpha+incs1
		beta<-beta+incs2
		gamma<-gamma+incs3
		R<-R+incsR
		Theta<-Theta+incsTheta
		ydata<-cbind(R*cos(Theta),R*sin(Theta))
		V<-var(ydata)
		ei<-eigen(V)
		ydata<-ydata%*%(ei$vectors)%*%diag(1/sqrt(ei$values))%*%t(ei$vectors)
		R<-sqrt(rowSums(ydata^2))
		Theta<-atan(ydata[,2]/ydata[,1])
