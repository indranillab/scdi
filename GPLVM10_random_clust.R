...
dL.dZ <- function(X, Z, l, alpha, sigma, sigma1,sigma2, K, dL.dK, Z.normal.prior, groups,cluster) {
  out <- Z
  for (i in 1:nrow(Z)) {
    for (j in 1:ncol(Z)) {
...
dL.dl <- function(X, Z, l, alpha, sigma, sigma1,sigma2, K, dL.dK, groups,cluster) {
  dK.dl <- dK.dl(Z, K, l,cluster)
  out <- sum(dL.dK * dK.dl)
  return(out)
}


dL.dalpha <- function(X, Z, l, alpha, sigma, sigma1,sigma2, K, dL.dK, groups,cluster) {
...
dL.dsigma <- function(X, Z, l, alpha, sigma, sigma1,sigma2, K, dL.dK, groups,cluster) {
...
dL.dsigma1 <- function(X, Z, l, alpha, sigma, sigma1, sigma2, K, dL.dK, groups,cluster) {
	...
	XX1<-array(0,dim<-c(dim(X)[1],length(table(cluster))))
	counter<-1
...
dL.dsigma2 <- function(X, Z, l, alpha, sigma, sigma1, sigma2, K, dL.dK, groups,cluster) {
	XX<-array(0,dim<-c(dim(Z)[1],length(table(groups))))
	counter<-1
	for(le in (unique(groups)))
	{
    ...
    dK.dsigma2<--2*sigma2*(1-XX%*%t(XX))*(XX1%*%t(XX1))*K

  out <- sum(dL.dK * dK.dsigma2)
  return(0)
}
...
gplvm.L.from.K <- function(Z, X, K, Z.normal.prior=TRUE,cluster) {
  X <- as.matrix(X)
  D <- ncol(X)
  N <- nrow(X)
  K.chol <- chol(K)
...
gplvm.L <- function(Z, X, l, alpha, sigma,sigma1,sigma2, Z.normal.prior=TRUE,groups,cluster) {
  K <- gplvm.SE(Z, l, alpha, sigma,sigma1,sigma2,groups,cluster)
  return(gplvm.L.from.K(Z, X, K, Z.normal.prior,cluster))
}
...
gplvm.f <- function(par, X,groups,cluster, Z.normal.prior) {
  ...
  sigma1 <- par[4]
  sigma2<-par[5]
  Z <- matrix(par[-(1:5)], nrow=nrow(X))
  gplvm.L(Z, X, l, alpha, sigma, sigma1,sigma2, Z.normal.prior,groups,cluster)
}
...
gplvm.gr <- function(par, X,groups, cluster,Z.normal.prior) {
  ...
  K <- gplvm.SE(Z, l, alpha, sigma,sigma1,sigma2,groups,cluster)
  dL.dK <- dL.dK(X, K)
  c(dL.dl(X, Z, l, alpha, sigma, sigma1, sigma2, K, dL.dK,groups,cluster),
    dL.dalpha(X, Z, l, alpha, sigma, sigma1, sigma2, K, dL.dK,groups,cluster),
    dL.dsigma(X, Z, l, alpha, sigma, sigma1, sigma2, K, dL.dK,groups,cluster),
    dL.dsigma1(X, Z, l, alpha, sigma, sigma1, sigma2, K, dL.dK, groups,cluster),
    dL.dsigma2(X, Z, l, alpha, sigma, sigma1, sigma2, K, dL.dK, groups,cluster),
    as.numeric(dL.dZ(X, Z, l, alpha, sigma, sigma1, sigma2, K, dL.dK, Z.normal.prior,groups,cluster)))
}
...
gplvm.hp.f <- function(par, Z, X,groups, cluster, Z.normal.prior) {
  ...
  sigma1<-par[4]
  sigma2<-par[5]
  gplvm.L(Z, X, l, alpha, sigma,sigma1,sigma2, Z.normal.prior=Z.normal.prior,groups, cluster)
}
...
#	}
  ...
  #Z<-array(rnorm((q*dim(X)[1]),0,1),dim<-c(dim(X)[1],q))
  par.init <- c(1, 1, 1,1,1)
  par.init.L <- gplvm.hp.f(par.init, Z=Z, X=X, groups, cluster, Z.normal.prior=Z.normal.prior)
  #first optimize the hyperparams for the initial Z
  for (i in seq(num.init.params)) {
    test.par.init <- rgamma(5, 1)
    test.par.init.L <- gplvm.hp.f(test.par.init, Z=Z, X=X,groups, cluster, Z.normal.prior=Z.normal.prior)
  ...
  init.sigma2<-as.numeric(optout[5])
  #init.sigma1<-sigma1_init

  cat("Starting params: l=", init.l, "; alpha=", init.alpha, "; sigma=", init.sigma, fill=TRUE)

  par <- c(init.l, init.alpha, init.sigma, init.sigma1, init.sigma2, as.numeric(Z))
  #par[4]<-10000
  ...
  while (its < iterations & convcode != 0) {
    ...
    par <- head(as.numeric(out), length(Z) + 5)
    l <- par[1]
    alpha <- par[2]
    sigma <- par[3]
    sigma1<-par[4]
    sigma2<-par[5]
    Z <- matrix(par[-(1:5)], ncol=q)
    if (q > 2) {
      pairs(Z,
            col=classes,
