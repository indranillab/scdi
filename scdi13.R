...
xxx1<-fit.gplvm1(X,8*max(K1,K2),iterations=100,Z.normal.prior=TRUE,Z.init="PCA",groups=groups,sigma1_init=0)
X<-xxx1$Z
data1<-X[(groups==0),]
data2<-X[(groups==1),]
...
xxx1<-fit.gplvm1(X,2*max(K1,K2),iterations=100,Z.normal.prior=TRUE,Z.init="PCA",groups=groups,sigma1_init=0)
...
ei<-eigen(v)
ei1<-eigen(v1)
mat<-(ei$vectors%*%diag((sqrt(ei$values)))%*%t(ei$vectors))%*%(ei1$vectors%*%diag((1/sqrt(ei1$values)))%*%t(ei1$vectors))
...
ei<-eigen(v)
ei2<-eigen(v2)
mat<-(ei$vectors%*%diag((sqrt(ei$values)))%*%t(ei$vectors))%*%(ei2$vectors%*%diag((1/sqrt(ei2$values)))%*%t(ei2$vectors))
...
iter<-1
	...
	xxx1<-fit.gplvm.prior(X,2*max(K1,K2),iterations=100,Z.init=Z2,Z.normal.prior=FALSE,groups=groups,sigma1_init=1,muu=muu,sigmaa=sigmaa,cluster=clust)
...
return(list(Z2=Z2,cluster=clust,groups=groups,l=xxx1$l,alpha=xxx1$alpha,sigma=xxx1$sigma,sigma1=xxx1$sigma1,dataa=dataa))
