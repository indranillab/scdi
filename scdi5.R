...
xxx<-fit.gplvm(X,30,iterations=100,Z.normal.prior=TRUE)
data<-xxx$Z
...
xxx1<-fit.gplvm1(X,2,iterations=100,Z.normal.prior=TRUE,groups=groups,sigma1_init=0)
...
while(eps>(0.001*(dim(Z2)[1])))
{
	...
	xxx1<-fit.gplvm.prior(X,2,iterations=100,Z.init=Z2,Z.normal.prior=TRUE,groups=groups,sigma1_init=1,muu=muu,sigmaa=sigmaa,cluster=clust)
