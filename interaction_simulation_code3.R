...
set.seed(0)
x1<-splatSimulate(params=newSplatParams(),method="groups",batchCells=c(200,100),group.prob=c(0.5,0.5),nGenes=1000,batch.facLoc=0.3,batch.facScale=0.3)
...
#data1<-assays(x1)$counts[,1:200][,(colData(x1)$Group[1:200]=="Group1")]
data2<-assays(x1)$counts[,201:300]
...
Gr<-c(rep("Group1",(dim(data1)[1])),as.vector(unlist(colData(x2)$Group))[201:300])
Gr<-c(as.vector(unlist(colData(x1)$Group))[1:100],as.vector(unlist(colData(x2)$Group))[201:300])
...
ccddii<-fftRtsne(data,rand_seed=0)
pdf("data_2.pdf")
plot(ccddii,col=cols,pch=(1+groups),xlab="Dimension I",ylab="Dimension II",main="Uncorrected Data")
dev.off()
...
library(batchelor)
xmnnCorrect<-mnnCorrect(t(data1original),t(data2original))


dat<-assays(xmnnCorrect)$corrected

library(tsne)
source("fast_tsne.R")
xdat<-fftRtsne(t(dat))

pdf("mnnCorrect_2.pdf")
plot(xdat,col=cols,pch=(1+groups),xlab="Dimension I",ylab="Dimension II",main="mnnCorrect")
dev.off()
...
library(batchelor)
xmnnCorrect<-mnnCorrect(t(data1),t(data2))
