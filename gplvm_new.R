tsne <-
function(X,ydata,sigma,sigma1,group, initial_config = NULL, k=2, initial_dims=30, perplexity=30, max_iter = 1000, min_cost=0, epoch_callback=NULL,whiten=TRUE, epoch=100 ){
	if ('dist' %in% class(X)) {
		n = attr(X,'Size')
		}else{
		X = as.matrix(X)
	...
	for(le in (unique(group)))
	{
		matt1<-matt1+sigma1[counter]*XX[,counter]%*%t(XX[,counter])
		counter<-counter+1
	}
	...
	for(le in (unique(group)))
	{
		diag(matt2)[group==le]<-diag(matt2)[group==le]+sigma[counter]
		counter<-counter+1
	}
