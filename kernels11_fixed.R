gplvm.SE.dist <- function(Z, l, alpha, sigma=0,sigma1,groups,b) { 
...
gplvm.SE <- function(Z, l, alpha, sigma,sigma1,groups,b) {
  gplvm.SE.dist(Z,
                l=l,
                alpha=alpha,
                sigma=sigma,
...
dK.dZij <- function(Z, K, i, j, l,b,groups) {
  out <- matrix(0, nrow=nrow(K), ncol=ncol(K))
  out[i, ] <- out[, i] <- (Z[, j] - Z[i, j]) / l^2 * K[i, ]
  return(out)
}

dK.dbi<-function(Z,K,i,l,b,groups)
{
	lst1<-which(groups==0)
	lst2<-which(groups==1)
	out <- matrix(0, nrow=nrow(K), ncol=ncol(K))
	for(k1 in lst1)
	{
		for(k2 in lst2)
		{
			out[k1,k2]<-2*(Z[k1,i]-Z[k2,i]-2*b[i])/l^2*K[k1,k2]
			out[k2,k1]<-2*(Z[k1,i]-Z[k2,i]-2*b[i])/l^2*K[k2,k1]
		}
	}
	return(out)
}
