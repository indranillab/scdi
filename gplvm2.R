tsne <-
function(X, group, initial_config = NULL, k=2, initial_dims=30, perplexity=30, max_iter = 1000, min_cost=0, epoch_callback=NULL,whiten=TRUE, epoch=100 ){
	if ('dist' %in% class(X)) {
		n = attr(X,'Size')
		}else{
		X = as.matrix(X)
	...
	for (iter in 1:max_iter){
		...
		#}
		K<-alpha*matt+(1/beta)*diag((dim(ydata)[1]))+alpha1*matt1
		K1<-solve(K)
