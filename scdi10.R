...
for(i in 1:(dim(dists)[1]))
{
	...
	{
	if(max(stat)>((length(x)-1)/sqrt(length(x))*sqrt(((qt(0.2/length(x),(length(x)-2)))^2/((qt(0.2/length(x),(length(x)-2)))^2+(length(x)-2))))))
	{
		j<-which(x==min(x))
		nngraph[i,j]<-1
	}
...
xxxnew1<-scdi(Z2[(groups==0),],Z2[(groups==1),],K1,K2)
