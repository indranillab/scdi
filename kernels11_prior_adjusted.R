gplvm.SE.dist <- function(dist.matrix, l1, alpha, sigma=0,sigma1,groups,b) { 
  ...
  K <- alpha^2 * exp(-dist.matrix^2/(2*l1^2)-sigma1^2*(1-XX%*%t(XX)))
  diag(K) <- diag(K) + sigma^2
  return(K)
}


gplvm.SE <- function(Z, l1, alpha, sigma,sigma1,groups,b) {
	Z1<-Z
	for(i in 1:(dim(Z)[1]))
	{
		if(groups[i]==1)
  ...
  gplvm.SE.dist(dist.matrix=as.matrix(dist(Z1)),
                l1=l1,
                alpha=alpha,
                sigma=sigma,
                sigma1=sigma1,
...
dK.dZij <- function(Z, K, i, j, l1,groups,b) {
	Z1<-Z
	for(i1 in 1:(dim(Z)[1]))
	{
		if(groups[i1]==1)
  ...
  lst1<-which(groups==0)
  lst2<-which(groups==1)
  out[i, ] <- out[, i] <- (Z1[, j] - Z1[i, j]) / l1^2 * K[i, ]
  return(out)
}

dK.db <- function(Z, K, i,l1,groups,b) {
  out<-array(0,dim<-dim(K))
  lst1<-which(groups==0)
  lst2<-which(groups==1)
  	for(k1 in lst1)
  	{
  		for(k2 in lst2)
  		{
  		    out[k1,k2]<-(-(Z[k2,i]-Z[k1,i]+b[i])/l1^2*K[k1,k2])
  		}
  	}
  return(out)
}

dK.dl <- function(Z, K, l1) {
