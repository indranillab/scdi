gplvm.SE.dist <- function(dist.matrix, l, alpha, sigma=0,sigma1,groups,cluster) { 
  ...
  K <- alpha^2 * exp(-dist.matrix^2/(2*l^2)-sigma1^2*(1-XX%*%t(XX))*(XX1%*%t(XX1)))
  diag(K) <- diag(K) + sigma^2
  return(K)
}


gplvm.SE <- function(Z, l, alpha, sigma,sigma1,groups,cluster) {
