...
dL.dZ <- function(X, Z, l, alpha, sigma, sigma1, K, dL.dK, Z.normal.prior, groups,ind,mumat,sigmat,pii) {
  out <- Z
  for (i in 1:nrow(Z)) {
    for (j in 1:ncol(Z)) {
...
gplvm.L <- function(Z, X, l1, alpha, sigma,sigma1, Z.normal.prior=TRUE,groups,mumat,sigmat,pii) {
	library(mvtnorm)
...
gplvm.f <- function(par, X,groups, Z.normal.prior,mumat,sigmat,pii) {
  ...
  gplvm.L(Z, X, l, alpha, sigma, sigma1, Z.normal.prior,groups,mumat,sigmat,pii)
}
...
gplvm.gr <- function(par, X,groups, Z.normal.prior,mumat,sigmat,pii) {
  ...
  c(dL.dl(X, Z, l, alpha, sigma, sigma1, K, dL.dK,groups),
    ...
    as.numeric(dL.dZ(X, Z, l, alpha, sigma, sigma1, K, dL.dK, Z.normal.prior,groups,ind,mumat,sigmat,pii))
  )
}
