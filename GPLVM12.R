...
gplvm.f <- function(par, X,groups,q,KK,mumat,sigmat,pii,l1,alpha,sigma,sigma1) {
  par1<-par
...
gplvm.gr <- function(par, X,groups,q,KK,mumat,sigmat,pii,l1,alpha,sigma,sigma1) {
  ...
  c(as.numeric(dL.dZ(X, Z, l1, alpha, sigma, sigma1, K, dL.dK1, groups,ind,mumat,sigmat,pii)))
}
...
par<-c(as.numeric(xxx$Z))

for(iter in 1:10)
{
	print(iter)
	l1<-l
 out <-optimx(par,gplvm.f,gplvm.gr,X=X,groups=groups,q=q,KK=KK,mumat=mumat,sigmat=sigmat,pii=pii,l1=l1,alpha=alpha,sigma=sigma,sigma1=sigma1,method="L-BFGS-B",control=list(trace=T,maximize=T,kkt=FALSE,maxit=100,starttests=FALSE))
 
 
 Z<-matrix(as.numeric(out)[1:(length(as.numeric(Z)))],nrow=(dim(X)[1]))
 plot(Z,col=cols)
