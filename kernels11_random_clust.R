gplvm.SE.dist <- function(dist.matrix, l, alpha, sigma,sigma1,sigma2,groups,cluster) { 
	...
	XX1<-array(0,dim<-c(dim(dist.matrix)[1],length(table(cluster))))
	counter<-1
  ...
  #K <- alpha^2 * exp(-dist.matrix^2/(2*l^2)-sigma1^2*(1-XX%*%t(XX))*(1-XX1%*%t(XX1))-sigma2^2*(1-XX%*%t(XX))*(XX1%*%t(XX1)))
  #K <- alpha^2 * exp(-dist.matrix^2/(2*l^2)-sigma2^2*(1-XX%*%t(XX))*(XX1%*%t(XX1)))
...
gplvm.SE <- function(Z, l, alpha, sigma,sigma1,sigma2,groups,cluster) {
