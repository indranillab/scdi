gplvm.SE.dist <- function(dist.matrix, l, alpha, sigma,sigma1,groups) { 
  ...
  K <- alpha^2 * exp(-dist.matrix^2/(2*l^2)-sigma1^2*(XX%*%t(XX))*dist.matrix^2)
  diag(K) <- diag(K) + sigma^2
  return(K)
}


gplvm.SE <- function(Z, l, alpha, sigma,sigma1,groups) {
...
dK.dZij <- function(Z, K, i, j, l,sigma1,groups) {
  ...
  out[i, ] <- out[, i] <- (Z[, j] - Z[i, j]) / l^2 * K[i, ]+sigma1^2*(Z[, j] - Z[i, j])*K[i, ]
  return(out)
}
