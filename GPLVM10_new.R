...
dL.dZ <- function(X, Z, l, alpha, sigma, sigma1, K, dL.dK, Z.normal.prior, groups) {
  out <- Z
  for (i in 1:nrow(Z)) {
    for (j in 1:ncol(Z)) {
      out[i, j] <- sum(dL.dK * dK.dZij(Z, K, i, j, l,sigma1,groups))
    }
  }
  if (Z.normal.prior) {
    out <- out - Z / 10^2
...
dL.dsigma1 <- function(X, Z, l, alpha, sigma, sigma1, K, dL.dK, groups) {
    ...
    dK.dsigma1<--2*sigma1*(XX%*%t(XX))*K*(as.matrix(dist(Z)^2))
...
gplvm.L <- function(Z, X, l, alpha, sigma,sigma1, Z.normal.prior=TRUE,groups) {
...
gplvm.f <- function(par, X,groups, Z.normal.prior) {
  ...
  gplvm.L(Z, X, l, alpha, sigma, sigma1, Z.normal.prior,groups)
}
...
gplvm.gr <- function(par, X,groups, Z.normal.prior) {
  ...
  c(dL.dl(X, Z, l, alpha, sigma, sigma1, K, dL.dK,groups),
    ...
    as.numeric(dL.dZ(X, Z, l, alpha, sigma, sigma1, K, dL.dK, Z.normal.prior,groups))
  )
}
...
gplvm.hp.f <- function(par, Z, X,groups, Z.normal.prior) {
  ...
  sigma <- par[3]
  sigma1<-par[4]
  gplvm.L(Z, X, l, alpha, sigma,sigma1, Z.normal.prior=Z.normal.prior,groups)
}
...
#	}
  ...
  while (its < iterations & convcode != 0) {
    out <- optimx(par,
                  ...
                  Z.normal.prior=Z.normal.prior,
                  method="L-BFGS-B",
                  lower=c(-Inf,-Inf,-Inf,0.1,rep(-Inf,(length(par)-4))),
                  control=list(trace=T,
                               maximize=T,
                               kkt=FALSE,
                               maxit=plot.freq,
