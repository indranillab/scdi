tsne <-
function(X,ydata,tau,group, initial_config = NULL, k=2, initial_dims=30, perplexity=30, max_iter = 1000, min_cost=0, epoch_callback=NULL,whiten=TRUE, epoch=100 ){
	if ('dist' %in% class(X)) {
		n = attr(X,'Size')
		}else{
		X = as.matrix(X)
	...
	matt1<-array(0,dim<-c(dim(XX)[1],dim(XX)[1]))
	counter<-1
	for(le in (unique(group)))
	{
		matt1<-matt1+tau[counter]*XX[,counter]%*%t(XX[,counter])
		counter<-counter+1
	}
	matt2<-array(0,dim<-c(dim(XX)[1],dim(XX)[1]))
	counter<-1
	for(le in (unique(group)))
	{
		diag(matt2)[group==le]<-diag(matt2)[group==le]+1
		counter<-counter+1
	}
	...
	for (iter in 1:max_iter){
		...
		#}
		K<-(1/beta)*matt1+(1/beta)*matt2+alpha*matt
		K1<-solve(K)
		grads<--(K1%*%matt%*%K1%*%ydata-(dim(X)[2])*K1%*%ydata)
		grads1<-(dim(X)[2])*sum(diag(K1%*%(matt)))-sum(diag(K1%*%ydata%*%t(ydata)%*%K1%*%matt))
		grads2<--(dim(X)[2])*sum(diag(K1%*%(matt1+matt2)))*tau/beta^2+sum(diag(K1%*%ydata%*%t(ydata)%*%K1%*%(matt1+matt2)))*tau/beta^2
		#grads3<-(dim(X)[2])*sum(diag(K1%*%(matt1)))-sum(diag(K1%*%ydata%*%t(ydata)%*%K1%*%matt1))
