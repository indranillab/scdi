...
dL.dZ <- function(X, Z, l, alpha, sigma, sigma1, K, dL.dK, Z.normal.prior, groups,muu,sigmaa,cluster) {
  ...
  {
  	siginv<-solve(sigmaa[(((k-1)*q+1):(k*q)),])
...
gplvm.L.from.K <- function(Z, X, K, Z.normal.prior=TRUE,muu,sigmaa,cluster) {
  ...
  for(k in 1:(dim(muu)[1]))
  {
  	below_ave<- function(x,me=muu[k],s1=sigmaa[(((k-1)*q+1):(k*q)),]) {  
    return(mahalanobis(x,me,s1))
    }
  ...
  for(k in 1:(dim(muu)[1]))
  {
  	varsum<-varsum+log(det(sigmaa[(((k-1)*q+1):(k*q)),]))*sum(cluster==k)
  }
  if (Z.normal.prior) {
