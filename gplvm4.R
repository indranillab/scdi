...
function(X, initial_config = NULL, k=2, initial_dims=30, perplexity=30, max_iter = 1000, min_cost=0, epoch_callback=NULL,whiten=TRUE, epoch=100 ){
	...
	for (iter in 1:max_iter){
		...
		#}
		K<-alpha*ydata%*%t(ydata)+(1/beta)*diag((dim(ydata)[1]))+10*gamma*matt1
		K1<-solve(K)
		...
		sig<-sd(ydata)
		ydata<-ydata+array(rnorm((dim(ydata)[1]*dim(ydata)[2]),0,sig/iter^2),dim<-dim(ydata))
		R<-NULL
		Theta<-NULL
		for(i in 1:(dim(ydata)[1]))
		{
			R[i]<-sqrt(sum((ydata[i,])^2))
			Theta[i]<-atan(ydata[i,2]/ydata[i,1])
		}
		#R<-R/sd(R)
...
function(X, initial_config = NULL, k=2, initial_dims=30, perplexity=30, max_iter = 1000, min_cost=0, epoch_callback=NULL,whiten=TRUE, epoch=100 ){
	...
	for (iter in 1:max_iter){
		...
		#	grads[i,] = apply(sweep(-ydata, 2, -ydata[i,]) * stiffnesses[,i],2,sum)
		#}
		K<-alpha*ydata%*%t(ydata)+(1/beta)*diag((dim(ydata)[1]))
		K1<-solve(K)
		grads<--(K1%*%matt%*%K1%*%ydata-(dim(X)[2])*K1%*%ydata)
		grads1<-(dim(X)[2])*sum(diag(K1%*%(ydata%*%t(ydata))))/2-sum(diag(K1%*%ydata%*%t(ydata)%*%K1%*%matt))/2
		grads2<--(dim(X)[2])*sum(diag(K1))/(2*beta^2)+sum(diag(K1%*%ydata%*%t(ydata)%*%K1))/(2*beta^2)
